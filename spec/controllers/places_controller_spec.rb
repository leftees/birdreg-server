  ...
  describe "index" do
    context "place" do
      it "should list all places sorted by country id, municipal and obsplace" do
        ...
        get :index
        expect(jsonapi.data[0]._id).to eq(place1.id.to_s)
        expect(jsonapi.data[1]._id).to eq(place2.id.to_s)
        expect(jsonapi.data[2]._id).to eq(place4.id.to_s)
        expect(jsonapi.data[3]._id).to eq(place5.id.to_s)
        expect(jsonapi.data[4]._id).to eq(place6.id.to_s)
        expect(jsonapi.data[5]._id).to eq(place3.id.to_s)
